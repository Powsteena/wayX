        ...
        <div className="max-w-6xl mx-auto bg-gray-300 p-6 rounded-lg shadow-lg">
            ...
            {rides.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full table-auto bg-gray-400 shadow-md rounded-lg">
                        ...
                        <tbody>
                            {rides.map((ride) => (
                                <tr key={ride._id} className="border-b hover:bg-gray-50">
                                    <td className="px-6 py-4 text-sm text-black">{ride.userId?.username || 'N/A'}</td>
                                    <td className="px-6 py-4 text-sm text-black">{ride.pickup?.address || 'Not provided'}</td>
                                    <td className="px-6 py-4 text-sm text-gblack">{ride.dropoff?.address || 'Not provided'}</td>
                                    <td className="px-6 py-4 text-sm text-black">{new Date(ride.scheduledDateTime).toLocaleString()}</td>
