    ...
    useEffect(() => {
        const fetchRides = async () => {
            try {
                ...
                const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/driver/scheduledrides`, {
    ...
    const handleAcceptRide = async (rideId) => {
        try {
            ...
            const response = await axios.patch(
                `${process.env.REACT_APP_BACKEND_URL}/api/driver/accept-ride/${rideId}`,
                {},
                { headers: { Authorization: `Bearer ${token}` } }
            );
